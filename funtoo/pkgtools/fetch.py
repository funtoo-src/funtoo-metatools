	...
	while attempts < pkgtools.model.fetch_attempts:
		...
		try:
			try:
				...
				pkgtools.model.log.info(f"Content Not Modified")
			...
			pkgtools.model.log.info(f"Writing {key_dict} to fetch_cache, body = {repr(result)[:100]}")
			await pkgtools.model.fetch_cache.write(key_dict=key_dict, body=result)
			return result
...
async def get_page(fetchable, max_age=None, encoding=None, refresh_interval=None, is_json=False):
	"""
	...
	pkgtools.model.log.debug(
		f'get_page {fetchable}, max_age={max_age}, refresh_interval={refresh_interval}, is_json={is_json}')
	...
	try:
		...
		if encoding:
			key_dict["encoding"] = encoding
		pkgtools.model.log.info(f"Attempting to retrieve cached result... {key_dict}")
		cached_result = await pkgtools.model.fetch_cache.read(
			key_dict=key_dict
		)
		pkgtools.model.log.info(f"Read from fetch cache: {key_dict}: headers: {cached_result['headers'] if 'headers' in cached_result else None} body: {repr(cached_result['body'])[:100]}")
	except CacheMiss:
		pkgtools.model.log.info("Cache MISS!")
		cached_result = None
