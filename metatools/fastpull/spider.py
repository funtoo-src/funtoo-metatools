	...
	async def http_fetch(self, request: FetchRequest, is_json=False, encoding=None, extra_headers=None) -> Tuple[Dict, str]:
		...
		async with self.acquire_fetch_slot(request):
			...
			try:
				...
				log.info(f'http_fetch: GET {request.url} extra_headers: {extra_headers}')
				response = await http_client.get(request.url, headers=headers, auth=auth, follow_redirects=True,
												 timeout=15)
				log.info(f"HTTP RESPONSE {response.status_code}")
